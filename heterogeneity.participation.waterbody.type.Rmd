...
apollo_beta=c( asc_noFish=0,
               ...
               sigma_asc_lakeMichigan=1,
               mu_asc_lakeSuperior=0,
               sigma_asc_lakeSuperior=1,
               mu_asc_river=0,
               sigma_asc_river=1,
               mu_b_coastal_lakeMichigan=0,
               sigma_b_coastal_lakeMichigan=1,
               mu_b_coastal_lakeSuperior=0,
               sigma_b_coastal_lakeSuperior=1,
               mu_b_coastal_river=0,
               sigma_b_coastal_river=1,
               b_weekend_holiday=0,
               b_age=0,
               b_female=0,
               b_weekend_age=0,
...
# specifying normal distribution
apollo_draws=list(
  interDrawsType="pmc",
  interNDraws=100,
  interUnifDraws=c(),
  interNormDraws=c("draws_asc_fish","draws_asc_lakeMichigan","draws_asc_lakeSuperior",
                   "draws_asc_river","draws_b_coastal_lakeMichigan","draws_b_coastal_lakeSuperior",
                   "draws_b_coastal_river"),
...
apollo_randCoeff=function(apollo_beta, apollo_inputs){
  ...
  randcoeff[["asc_fish"]]=mu_asc_fish + sigma_asc_fish*draws_asc_fish
  randcoeff[["asc_lakeMichigan"]]=mu_asc_lakeMichigan + sigma_asc_lakeMichigan*draws_asc_lakeMichigan
  randcoeff[["asc_lakeSuperior"]]=mu_asc_lakeSuperior + sigma_asc_lakeSuperior*draws_asc_lakeSuperior
  randcoeff[["asc_river"]]=mu_asc_river + sigma_asc_river*draws_asc_river
  randcoeff[["b_coastal_lakeMichigan"]]=mu_b_coastal_lakeMichigan +
    sigma_b_coastal_lakeMichigan*draws_b_coastal_lakeMichigan
  randcoeff[["b_coastal_lakeSuperior"]]=mu_b_coastal_lakeSuperior +
    sigma_b_coastal_lakeSuperior*draws_b_coastal_lakeSuperior
  randcoeff[["b_coastal_river"]]=mu_b_coastal_river + sigma_b_coastal_river*draws_b_coastal_river
...
apollo_sink()
```
successful model fit, checking distribution (and correlations) of random parameters
...
```
```{r}
asc_fish<-conditionals$asc_fish

hist(asc_fish$post.mean)

list2env(conditionals, envir=.GlobalEnv)
# each param is now in env

params<-data.frame(ID=asc_fish$ID,
                   asc_fish=asc_fish$post.mean,
                   asc_lakeMichigan=asc_lakeMichigan$post.mean,
                   asc_lakeSuperior=asc_lakeSuperior$post.mean,
                   asc_river=asc_river$post.mean,
                   b_coastal_lakeMichigan=b_coastal_lakeMichigan$post.mean,
                   b_coastal_lakeSuperior=b_coastal_lakeSuperior$post.mean,
                   b_coastal_river=b_coastal_river$post.mean)

# correlation matrix
library(PerformanceAnalytics)

chart.Correlation(params[,-1], histogram=T)
```

ah, model specification didn't allow for them to be correlated. But the real story is how this may influence model predictions. 

```{r}
model=apollo_loadModel("MMNL_participate_waterbodyType")
...
```
Predicting higher peak summer participation when we incorporate individual heterogeneity
...
```
and a lot more variation among individuals, obviously

What does that mean for annual estimates? 


- nothing on rivers, so leaving them out
```{r}
total.obs<-10000000+155581+80351

inland<-10000000/total.obs

great<-(155581+80351)/total.obs
inland
great

comparison.df<-data.frame(type=rep("observed"), 
                          waterbodyType=c("Inland lakes","Lake Michigan", "Lake Superior", "Rivers"),
                          nTrips=c(10000000, 155581, 80351, NA))%>%
...
'observed' comes from 2001 DNR report on a mail survey--around 11000000 annual trips on inland lakes *and* rivers, both resident and nonresident
...
```
wow, that's weirdly close. Don't get excited though, the 'observations' for inland lakes aren't totally comparable. There are known issues creeling the Great Lakes in WI, which might explain the 'overprediction' of fishign effort there by this model, but who knows