...
anglerSim<-rbind.data.frame(anglerSim.1, anglerSim.2)%>%
  mutate(SurveyID=seq(1:nrow(anglerSim)))
...
predictions_meanAge<-predictions_cov%>%
  cbind.data.frame(dplyr::select(pred_data, month_Jan:month_Dec, weekendHoliday))

predictions_distinct<-predictions_meanAge%>%
  select(noFish:weekendHoliday)%>%
  distinct()
...
pop<-read_csv(here::here("data","county.pop.csv"))%>%
  ...
  # this is where I left off; plan is to join probabilities of choice by month, weekendHoliday, coastal county, and gender. Then multiply prob by stratum population, multiply that by days in the stratum, and add across strata to get annual estimates
  ...
  mutate(date=rep(yearDates, 8),
         day=wday(date, label=T),
         month=month(date, label=T))%>%
  dummy_cols(select_columns="month", remove_first_dummy=F, remove_selected_columns=T)%>%
  mutate(weekendHoliday=ifelse(day%in%c("Sat","Sun") | date%in%holidays, 1, 0))%>%
  # to prep for the join, I now want to group counties by coastal/noncoastal and add up their total populations. That probably actually wants to happen earlier in this pipe
  
  
  
  # now join the choice probability predictions by month, weekendHoliday, coastal vars, and gender
  left_join(predictions_distinct, by=c("month_Jan","month_Feb","month_Mar","month_Apr","month_May",
                                       "month_Jun","month_Jul","month_Aug","month_Sep","month_Oct",
                                       "month_Nov","month_Dec","weekendHoliday","coastal_lakeMichigan",
                                       "coastal_lakeSuperior","coastal_river","gender"))

# so let's expand pop into a year. Add dates, month, weekend/weekday
